...
checksum-mismatch-try-next-url = 软件包 {$c} 校验和验证失败，将使用下一个镜像源重试 ...
checksum-mismatch-retry = 软件包 {$c} 校验和验证失败，将重试 {$retry} 次 ...
can-not-get-source-next-url = 无法下载软件包 {$e}，将使用下一个镜像源重试 ...
checksum-mismatch = 位于 {$dir} 的软件包文件 {$filename} 校验和验证失败。
...
earlier-signature = InRelease 文件无效：系统时间早于内联签名时间戳。
expired-signature = InRelease 文件无效：内联签名已过期。
