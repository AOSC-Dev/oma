...
cnf-similar-match = The following packages provide similarly named commands:
cnf-command-list = (similar command(s): { $cmds })
cnf-command-separator = { ", " }
cnf-install-tip = To use { $kw }, install the package providing this command:
cnf-install-tip-multi = To use { $kw }, pick the package you want to install from the list above:
cnf-install-tip-similar = Tip: You may install the package listed above with oma install <name>; to view all packages providing similarly named commands, use oma provides --bin { $query }.
...
checksum-mismatch-retry = Checksum verification failed for { $c } (retried { $retry } times) ...
timeout-retry = Download timeout failed for { $c } (retried { $retry } times) ...
can-not-get-source-next-url = Retrying with the next available mirror ...
fallback-to-by-name-path = { $c }: by-hash file is not available, falling back to the traditional path ...
...
oma = oma
another-oma-is-running = Another oma (Pid: { $pid }) instance is running: { $s }
another-oma-is-running-without-status = Another oma (Pid: { $pid }) instance is running.
table-name = Name
...
oma-archive-lock = Unable to Download packages.
oma-refresh-lock-dueto = { $exec } ({ $pid }) has acquired the lock to the database.
oma-archive-lock-dueto = { $exec } ({ $pid }) has acquired the lock to the archive (local package cache).
...
set-mirror-order-prompt = Use [Space] to select a mirror, then [↑/↓] to re-order, [Enter] to apply changes
...
history-tui-command = Command
history-tui-status = Status
history-tui-ops = Ops
history-tui-date = Date
