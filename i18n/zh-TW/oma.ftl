...
all-speedtest-failed = 所有鏡像站速度測試均失敗，請檢查您的網路連線。
failed-to-parse-file = 無法解析檔案 { $p }。
mirror-not-found = 找不到鏡像站 { $mirror }。請使用 `oma mirror' 指令選擇可用鏡像站。
failed-to-serialize-struct = BUG: 解析設定檔時發生未知錯誤。
failed-to-write-file = 無法寫入檔案 { $p }。
failed-to-create-file = 無法建立檔案 { $p }。
mirror-name = 鏡像站
mirror-score = 速度
mirror-speedtest-start = 即將開始鏡像站速度測試……
speedtest-complete = 鏡像站測試完成。
