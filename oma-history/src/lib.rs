...
impl History {
    ...
    pub fn find_history_topics_status_by_id(
        ...
        let mut query_history_table = self
            .connection
            .prepare(
    ...
    pub fn find_history_by_id(&self, id: i64) -> HistoryResult<HistoryEntryInner> {
        let mut query_history_table = self.connection
        ...
        let mut query_install_table = self.connection.prepare(r#"SELECT package_name, old_version, new_version, old_size, new_size, download_size, arch, operation FROM "history_install_package_oma_1.14"
    WHERE history_id = (?1)"#)
    .map_err(HistoryError::ExecuteError)?;
        ...
        let mut query_remove_table = self
            .connection
            .prepare(
    ...
    WHERE history_id = (?1)"#,
        ...
        let mut query_remove_details_table = self.connection
        .prepare(
    ...
    WHERE history_id = (?1)"#,
        ...
        if let Some(i) = res_iter.next() {
            let (is_success, disk_size, total_download_size) =
                i.map_err(HistoryError::ParseDbError)?;
    ...
    pub fn last_upgrade_timestamp(&self) -> HistoryResult<i64> {
