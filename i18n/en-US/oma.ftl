...
clap-undo-help = Undo a package change to the system
...
clap-color-help = Toggle color preferences for oma output
clap-debug-help = Run oma with debug output
clap-download-threads-help = Set the number of parallel download threads (default as 4)
clap-dry-run-help = Run oma in "dry-run" mode
clap-dry-run-long-help = This is useful for testing changes and operations without making changes to the system
...
clap-force-unsafe-io-help = Install package(s) without fsync(2) (no periodic data sync to the storage device, DANGEROUS but fast)
...
clap-no-autoremove-help = Do not remove unneeded package(s)
clap-no-bell-help = Don't ring the terminal bell as oma completes the transaction
clap-no-check-battery-help = Do not check for power status (whether the system is running on battery)
...
clap-topics-opt-out-help = Withdraw from one or more topic(s) and rollback to stable versions
clap-topics-always-write-status-help = Always write topic configuration status (even if enrollment fails)
clap-topics-only-apply-sources-list-help = Only write to source configuration file(s), do not perform other system changes
clap-topics-yes-long-help = Note that this parameter requires on the `--opt-out' or `--opt-in' parameter and is otherwise invalid
clap-tree-packages-help = Package(s) to query dependency(ies) for
clap-tree-reverse-help = Print the reverse dependency(ies) for the specified packages
clap-tree-depth-help = Maximum query level for dependencies (default: 5, RAM REQUIREMENT INCREASES EXPONENTIALLY!)
clap-why-packages-help = Package(s) to query reverse dependency(ies) for
