...
execute-cmd-fail = 無法執行 `{ $cmd }' 指令：{ $e }。
# history
...
cnf-exact-match = oma 找到了提供該指令的軟體套件：
cnf-similar-match = 如下軟體套件提供了名稱相似的指令：
cnf-command-list = (相似指令: { $cmds })
cnf-command-separator = { ", " }
cnf-install-tip = 要使用 { $kw }，請執行以安裝對應的軟體套件：
cnf-install-tip-multi = 要使用 { $kw }，請從以上軟體套件中選擇您想安裝的一個：
cnf-install-tip-similar = 提示：您可以使用 oma install <軟體套件名稱> 安裝上面列出的軟體套件，如需檢視所有提供相似指令的軟體套件，請使用 oma provides --bin { $query }。
...
checksum-mismatch-retry = 檔案 { $c } 完整性驗證失敗（已重試 { $retry } 次）...
timeout-retry = 檔案 { $c } 下載超時（已重試 { $retry } 次）...
can-not-get-source-next-url = 將使用下一個鏡像源重試 ...
fallback-to-by-name-path = { $c }: by-hash 檔案不可用，回落到傳統路徑 ...
...
tui-start-7 = 結束
oma = 小熊貓套件管理器
another-oma-is-running = 有另一個 oma (Pid: { $pid }) 實例正在執行中：{ $s }
table-name = 套件名稱
table-version = 版本
table-size = 大小
table-detail = 註記
...
set-mirror-order-prompt = 按 [Space] 選中鏡像源，然後使用 [↑/↓] 修改優先順序，[Enter] 套用更改
...
upgrade-manual-held-tips = { $count } 個軟體套件被標記為版本鎖定，未被升級。
upgrade-resolver-held-tips = { $count } 個軟體套件被依賴解析器標記為版本鎖定，未被升級。
...
clap-fix-dpkg-status-help = 修復 dpkg 的「損壞 (broken)」狀態
...
oma-mirror-outdated = { $mirror }: 存在延遲 - { $duration }
clap-no-config-help = 不使用設定檔，使用預設設定
no-config-warning = 因執行 oma 時使用了 --no-config 選項，將使用預設設定。
clap-noclean-help = 不要清理本機軟體套件快取
clap-mirror-latency-help = 測試鏡像同步延遲
mirror-latency = 延遲
another-oma-is-running-without-status = 有另一個 oma (Pid: { $pid }) 實例正在執行中。
oma-archive-lock = 無法下載軟體套件。
clap-user-agent-help = 指定用於 HTTP 請求的 User-Agent 標頭字串
history-detail = 顯示選中的歷史記錄詳情
undo-detail = 復原選中的歷史記錄
history-tui-command = 指令
history-tui-status = 狀態
history-tui-ops = 操作
history-tui-date = 日期
history-tui-installation-succeeded = 操作成功完成
history-tui-installation-failed = 操作失敗
