...
clap-options = 选项
clap-usage = 用法
clap-about = 小熊猫包管理是一款针对 APT 软件仓库，专为用户友好与防呆设计的软件包管理器
clap-install-help = 从仓库中安装软件包
clap-upgrade-help = 升级系统中安装的软件包
clap-download-help = 从软件仓库下载软件包
clap-remove-help = 删除指定软件包
clap-refresh-help = 刷新软件仓库库元数据/目录
clap-show-help = 显示指定软件包的相关信息
clap-search-help = 搜索软件仓库中可用的软件包
clap-files-help = 列出指定软件包中的文件
clap-provides-help = 搜索路径中提供特定字符串的软件包
clap-fixbroken-help = 解决系统中被破坏的依赖关系
clap-pick-help = 安装软件包的特定版本
clap-mark-help = 标记一个或多个软件包的状态
clap-depends-help = 列出一个或多个软件包的依赖关系
clap-rdepends-help = 列出指定软件包的反向依赖关系
clap-clean-help = 清除已下载软件包的缓存
clap-history-help = 显示系统中软件包更改的历史
clap-undo-help = 撤销系统更改
clap-tui-help = TUI 界面
clap-version-help = 显示小熊猫包管理的版本号
clap-topics-help = 开启或关闭主题制测试源
clap-mirror-help = 开启或关闭镜像源
clap-size-analyzer-help = 软件包磁盘占用分析器
clap-tree-help = 显示树形可视化依赖关系图
clap-why-help = 分析为什么指定的软件包已安装
clap-debug-help = 以调试模式运行小熊猫包管理
